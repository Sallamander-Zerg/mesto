...
const sectionCards = new Section(
  {
    items: initialCards,
    renderer: (item) => {
      const newCard = createCard(item.text, item.alt, item.src,handleCardClick);
...
const FormSubmitAdd = (fieldAddValues)=>{
  const newCard = createCard(fieldAddValues.fieldMesto,fieldAddValues.fieldMesto,fieldAddValues.fieldSrc,handleCardClick)
  sectionCards.additem(newCard)
}
